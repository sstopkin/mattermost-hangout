...
module.exports = function (app) {
	app.post('/', function (req, res) {
		...
		google.createHangoutMeeting(createMeetingPayload, function (err, event) {
			...
			message = message.replace('{link}', event.data.hangoutLink);
			message = message.replace('{summary}', event.data.summary);

			log.info("Created Call for user " + req.body.user_name + " at " + event.data.hangoutLink + "!");
